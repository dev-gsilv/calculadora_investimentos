...
const templateRbac = async () => {
  ...
  if (role == "usuario") {
    ...
    if (usuarioId != resource.ID) {
      return res
        .status(403)
        .json({ Erro: "Você não tem permissão para ver este recurso!" });
    }
...
export const create = async (req, res) => {
  try {
    ...
    try {
      ...
      res
        .status(500)
        .json({ msg: "Erro no servidor. Por favor, tente novamente!" });
    }
  } catch (e) {
    console.error(e);
    res
      .status(400)
      .json({ msg: "Requisição inválida. Por favor, tente novamente." });
  }
};
...
export const getAll = async (req, res) => {
  try {
    ...
    if (role == "usuario") {
      return res
        .status(403)
        .json({ Erro: "Voce nao tem permissao para fazer esta requisiçao!" });
    }
  } catch (e) {
    console.error(e);
    res
  ...
  if (role == "admin") {
    try {
      ...
      try {
        ...
        res
          .status(200)
          .json({ msg: "Usuario removido!", Usuario_ID: query._id });
      } catch (e) {
        console.error(e);
        res
          .status(500)
